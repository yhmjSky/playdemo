...
import com.xcq.playdemo.yhmjplayer.YhmjSkyPlayer;
 ...
 * @time 2016-9-19
 */
public class SuperVideoDetailsActivity extends AppCompatActivity implements View.OnClickListener, YhmjSkyPlayer.OnNetChangeListener {

    private YhmjSkyPlayer player;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_yhmjsky_video_layout);
        initData();
        initView();
        initPlayer();
    }
    ...
    private void initPlayer(){
        player = (YhmjSkyPlayer) findViewById(R.id.view_super_player);
        if(isLive){
            player.setLive(true);//设置该地址是直播的地址
        }
        player.setNetChangeListener(true)//设置监听手机网络的变化
                .setOnNetChangeListener(this)//实现网络变化的回调
                .onPrepared(new YhmjSkyPlayer.OnPreparedListener() {
                    @Override
                ...
                }).onInfo(new YhmjSkyPlayer.OnInfoListener() {
                    @Override
                ...
                }).onError(new YhmjSkyPlayer.OnErrorListener() {
                    @Override
                ...
                .play(url);//开始播放视频
        player.setScaleType(YhmjSkyPlayer.SCALETYPE_FITXY);
